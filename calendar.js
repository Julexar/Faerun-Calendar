                ...
                case "!cal":
                    switch (args[1]) {
                        ...
                        case 'settime':
                            ...
                            const minute = parseInt(args[4]);
                            if (isNaN(hour) || isNaN(minute)) return sendChat("Faerûn Calendar", "Please input a valid number for the hour and minute.");
                        ...
                        case 'advance':
                            ...
                            if (!["Short Rest", "Long Rest", "Minute", "Hour", "Day", "Week", "Month", "Year"].includes(type)) return sendChat("Faerûn Calendar", "Please input a valid type.");
...
function calendarMenu() {
    ...
    switch (weather) {
        default:
            switch (moon) {
                default:
                    sendChat("Faerûn Calendar", `/w gm <div ${calendar.style.divMenu}>` + //--
...
function showCal() {
    ...
    switch (weather) {
        default:
            switch (moon) {
                default:
                    sendChat('Faerûn Calendar', `<div ${calendar.style.divMenu}>` + //--
...
function createAlarm(title, date, time, message) {
    ...
    sendChat('Faerûn Calendar', `/w gm Alarm #${state.alarms.length-1} created!\n` + //--
        `Title: ${title}\n` + //--
        `Date: ${date}\n` + //--
        `Time: ${time}\n` + //--
        `Message: ${message}`
    ...
    for (let i=0;i<state.alarms.length;i++) {
        ...
        if (alarm.hour) {
            if (alarm.year === state.calendar.year && 
                ...
                (alarm.day >= state.calendar.day && !(alarm.day >= state.calendar.day+7)) && 
                (alarm.hour >= state.calendar.hour && !(alarm.hour >= state.calendar.hour+12)) && 
                (alarm.minute >= state.calendar.minute && !(alarm.minute >= state.calendar.minute+30))
            ) {
                sendChat('Faerûn Calendar', `/w gm Alarm #${i} triggered!\n` + //--
                    `Title: ${alarm.title}\n` + //--
